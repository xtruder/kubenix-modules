...

{
  ...
  config = {
    ...
    opts = {
      listeners = "PLAINTEXT://\${HOSTNAME}.${name}:9093";
      "zookeeper.connect" = concatStringsSep "," config.args.zookeeper.servers;
    ...
    kubernetes.api.statefulsets.kafka = {
      spec = {
        serviceName = name;
