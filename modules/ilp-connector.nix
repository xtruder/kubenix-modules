          ...
          spec = {
            containers = {
              connector = {
                ...
                env = {
                  CONNECTOR_PLUGINS.value = ""; # concatStringsSep "," (unique (mapAttrsToList (n: v: v.plugin) config.args.accounts));
                  CONNECTOR_ENV.value = config.args.environment;
                  CONNECTOR_ILP_ADDRESS.value = config.args.ilpAddress;
                  CONNECTOR_ILP_ADDRESS_INHERIT_FROM = mkIf (config.args.ilpAddressInheritFrom != "") {
                    value = config.args.ilpAddressInheritFrom;
                  };
                  CONNECTOR_ACCOUNTS.value = builtins.toJSON (moduleToAttrs config.args.accounts);
                  CONNECTOR_DEFAULT_ROUTE.value = config.args.defaultRoute;
                  CONNECTOR_ROUTES.value = builtins.toJSON config.args.routes;
                  CONNECTOR_SPREAD.value = toString config.args.spread;
                  CONNECTOR_MIN_MESSAGE_WINDOW.value = toString config.args.minMessageWindow;
                  CONNECTOR_MAX_HOLD_TIME.value = toString config.args.maxHoldTime;
                  CONNECTOR_ROUTE_BROADCAST_ENABLED.value = boolToString config.args.routing.broadcast.enabled;
                  CONNECTOR_ROUTE_BROADCAST_INTERVAL = mkIf (config.args.routing.broadcast.enabled) {
                    value = toString config.args.routing.broadcast.interval;
                  };
                  CONNECTOR_ROUTE_CLEANUP_INTERVAL.value = toString config.args.routing.cleanup;
                  CONNECTOR_ROUTE_EXPIRY.value = toString config.args.routing.expiry;
                  CONNECTOR_ROUTING_SECRET = k8s.secretToEnv config.args.routing.secret;
                  CONNECTOR_BACKEND.value = config.args.ratesBackend.type;
                  CONNECTOR_BACKEND_CONFIG = mkIf (config.args.ratesBackend.config != null) {
                    value = builtins.toJSON config.args.ratesBackend.config;
                  };
                  CONNECTOR_STORE.value = config.args.store.type;
                  CONNECTOR_STORE_PATH.value = "/storage";
                  CONNECTOR_STORE_CONFIG = mkIf (config.args.store.config != null) {
                    value = builtins.toJSON config.args.store.config;
                  };
                  CONNECTOR_MIDDLEWARES.value = builtins.toJSON config.args.middleware.enable;
                  CONNECTOR_DISABLE_MIDDLEWARE.value = builtins.toJSON config.args.middleware.disable;
                  CONNECTOR_REFLECT_PAYMENTS.value = boolToString config.args.reflectPayments;
                  CONNECTOR_INITIAL_CONNECT_TIMEOUT.value = toString config.args.initialConnectTimeout;
                  CONNECTOR_ADMIN_API.value = boolToString config.args.adminApi.enable;
                  CONNECTOR_ADMIN_API_PORT = mkIf (config.args.adminApi.enable) {
                    value = config.args.adminApi.port;
                  };
                  CONNECTOR_ADMIN_API_HOST = mkIf (config.args.adminApi.enable) {
                    value = config.args.adminApi.host;
                  };
                  CONNECTOR_COLLECT_DEFAULT_METRICS.value = boolToString config.args.collectDefaultMetrics;
                } // (mapAttrs (name: value: k8s.secretToEnv value.secret) config.args.accountsSecrets);
                ...
                ports = (map (port: {containerPort = port;}) config.args.extraConnectorPorts)
                    ++ (optionals config.args.adminApi.enable [{ containerPort = config.args.adminApi.port; }]);
              };
            };
          };
        };
      ...
      ilp-connector = {
        ...
        spec = {
          ...
          ++ (map (port: {
            ...
            port = port;
            targetPort = port;
          }) config.args.extraConnectorPorts)
          ++ (optionals config.args.adminApi.enable [{
            name = "admin";
            port = config.args.adminApi.port;
            targetPort = config.args.adminApi.port;
          }]);
        };
      };
    };
