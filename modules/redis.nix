...
let
  ...
  redisConfig = ''
    ...
    appendonly ${b2s config.args.appendonly}
...
in {
  ...
  options.args = {
    ...
    storage = {
      size = mkOption {
        ...
        type = types.str;
        default = "${toString (config.args.nodes.memory * 3)}Mi";
      };
  ...
  config = {
    ...
    kubernetes.api.configmaps.redis.data."redis.conf" = redisConfig;

    kubernetes.api.deployments.redis-proxy = {
        ...
        template = {
          ...
          spec = {
            ...
            containers.redis-proxy = {
              ...
              ] ++ (optionals (config.args.proxy.command != "") ["-redis-command" config.proxy.command]);
              image = config.args.args.proxy.image;
              ports = [{
                name = "redis";
                containerPort = 6379;
              }];
    ...
    kubernetes.api.poddisruptionbudgets.redis-proxy = {
      metadata.name = "${name}-proxy";
      metadata.labels.app = name;
      metadata.labels.component = "redis-proxy";
    ...
    kubernetes.api.statefulsets.redis-node = {
      ...
      spec = {
        ...
        replicas = config.args.nodes.replicas;
        template = {
          metadata.labels.app = name;
          metadata.labels.component = "redis-node";
          ...
          spec = {
            ...
            containers.redis-metrics = {
              ...
              env.REDIS_PASSWORD = mkIf (config.args.password != null) (secretToEnv config.args.password);
              ports = [{
                name = "metrics";
                containerPort = 9121;
              }];
            ...
            containers.redis-health = {
              ...
              env.REDIS_PASS = mkIf (config.args.password != null) (secretToEnv config.args.password);
              ports = [{
                name = "health";
                containerPort = 5000;
              }];
            ...
            containers.redis = {
              command = ["bash" "-ec" ''
                ...
                if [[ $ordinal -eq 0  ]]; then
                  ...
                  exec redis-server /etc/redis/redis.conf \
                    ...
                    ${optionalString (config.args.password != null) "--requirepass $REDIS_PASSWORD"} \
                    ${optionalString (config.args.password != null) "--masterauth $REDIS_PASSWORD"}
                else
                  exec redis-server /etc/redis/redis.conf \
              ...
              resources.requests = {
                cpu = config.args.nodes.cpu;
                memory = "${toString (config.args.nodes.memory + 50)}Mi";
              };
              resources.limits.cpu = config.args.nodes.limit;
    ...
    kubernetes.api.poddisruptionbudgets.redis-node = {
      metadata.name = "${name}-node";
      metadata.labels.app = name;
      metadata.labels.component = "redis-node";
      spec.maxUnavailable = 1;
    ...
    kubernetes.api.statefulsets.redis-sentinel = {
      ...
      spec = {
        ...
        replicas = config.args.sentinel.replicas;
        ...
        template = {
          ...
          spec = {
            ...
            containers.redis-sentinel = {
              ...
              ] ++ optionals (config.args.password != null) ["--sentinel" "auth-pass" "redis" "$(REDIS_PASSWORD)"];
              env.REDIS_PASSWORD = mkIf (config.args.password != null) (secretToEnv config.args.password);
              env.POD_NAME.valueFrom.fieldRef.fieldPath = "metadata.name";
              ports = [{
                name = "redis-sentinel";
              ...
              readinessProbe = {
                exec.command = ["sh" "-c" ''
                  redis-cli ${optionalString (config.args.password != null) "-a $REDIS_PASSWORD"} -p 26379 ping | grep PONG
                ''];
              ...
              livenessProbe = {
                exec.command = ["sh" "-c" ''
                  redis-cli ${optionalString (config.args.password != null) "-a $REDIS_PASSWORD"} -p 26379 info | grep master0 | grep ok
                ''];
    ...
    kubernetes.api.poddisruptionbudgets.redis-sentinel = {
      metadata.name = "${name}-sentinel";
      metadata.labels.app = name;
      metadata.labels.component = "redis-sentinel";
      spec.maxUnavailable = 1;
    ...
    }) (range 0 (config.args.sentinel.replicas - 1))) ++ (map (i: {
    ...
    }) (range 0 (config.args.nodes.replicas - 1))));
  };
}