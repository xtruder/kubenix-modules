{ name, args, config, lib, pkgs, kubenix, ...}:
...
let
  ...
  dashdConfig = ''
    ...
    testnet=${b2s args.testnet}

    # Run a regression test network
    regtest=${b2s args.regtest}
    ...
    #
    # JSON-RPC options (for controlling a running Bitcoin/dashd process)
    #

    # server=1 tells Bitcoin-Qt and dashd to accept JSON-RPC commands
    server=${b2s args.server}
    ...
    # You can use Bitcoin or dashd to send commands to Bitcoin/dashd
...
in {
  ...
  options.submodule.args = {
    ...
    storage = {
      ...
      size = mkOption {
        ...
        type = types.str;
        default = if args.testnet || args.regtest then "30Gi" else "250Gi";
      };
    };
  };
  ...
  config = {
    ...
    docker.images.dashd.image = pkgs.dockerTools.pullImage {
      imageName = "helidium/dashd";
      imageDigest = "sha256:a1ac1d220a8a719ef3b1684410a29b44f4c7abdad9e2d899830c2808aec30413";
      sha256 = "0isw0zvr1q6fyw339wkmydr82nah8xgj0ill19la4dkwzbyffk2d";
      finalImageName = "dashd";
      ...
      spec = {
        ...
        template = {
          ...
          spec = {
            initContainers = [{
              ...
              command = ["sh" "-c" "cp /config/bitcoin.conf /home/dash/.dashcore"];
              volumeMounts = [{
                name = "config";
                mountPath = "/config";
              } {
                name = "data";
                mountPath = "/home/dash/.dashcore/";
              }];
            }];
            containers.dashd = {
              image = config.docker.images.dashd.path;
              imagePullPolicy = "IfNotPresent";
        ...
        volumeClaimTemplates = [{
          ...
          spec = {
            ...
            storageClassName = mkIf (args.storage.class != null) args.storage.class;
